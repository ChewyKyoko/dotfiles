...
Item {
    ...
    function saveRecentsToDisk() {
        let rawChars = backend.recentItems.map(function(item) { return item.emoji; });
            ...
            onStreamFinished: {
                try {
                    ...
                    if (Array.isArray(savedChars)) {
                        var recentItems = [];
                        for (var ci = 0; ci < savedChars.length; ci++) {
                            var ch = savedChars[ci];
                            for (var ai = 0; ai < backend.allItems.length; ai++) {
                                if (backend.allItems[ai].emoji === ch) {
                                    recentItems.push(backend.allItems[ai]);
                                    break;
                                }
                            }
                        }
                        backend.recentItems = recentItems;
                    }
                } catch (e) {
