...
Item {
    ...
    function updateSearch() {
        let baseList = backend.currentTab === 0 ? backend.allItems : backend.allItems.filter(function(item) { return backend.pinnedRaws.includes(item.raw); });

        if (backend.searchText.trim() === "") {
            backend.filteredItems = baseList.map(function(item) {
                return {
                    raw: item.raw,
    ...
    function clearUnpinnedHistory() {
        let unpinned = backend.allItems.filter(function(item) { return !backend.pinnedRaws.includes(item.raw); }).map(function(item) { return item.raw; });
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                backend.pinnedRaws = backend.pinnedRaws.filter(function(pinnedRaw) {
                    return backend.allItems.some(function(item) { return item.raw === pinnedRaw; });
                });
